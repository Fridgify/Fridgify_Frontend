...
void main() async {
  ...
  setUp(() async {
    ...
    mockClient = new MockClient((request) async {
      ...
      var endpoint = request.url.toString().replaceAll('${Repository.baseURL}auth/', '');
  ...
  group('Login', () {
    ...
    test('gets a api token', () async {
      ...
      expect(Future.value('Api token'), completion(await Repository.sharedPreferences.get('clientToken')));
    });
  ...
  group('Register', () {
    ...
    test('gets a return type other than 201', () async {
      expect(() async => completion(await authService.register()), throwsA(predicate((error) => error is FailedToFetchClientTokenException)));
    });
  ...
  group('fetchApiToken', () {
    ...
    test('gets a return type other than 201', () async {
      await Repository.sharedPreferences.setString('clientToken', 'Other than 201');
    ...
    test('gets a token and sets it', () async {
      await Repository.sharedPreferences.setString('clientToken', 'valid token');
